...
## v3.14（2026-09-12）— 增强图改后台渲染：「保存并释放」不再等画图

**用户诉求**：标注当前这张时把后面几份缓存起来，保存并释放之后能快速进入下一张。

**先量，再设计** —— 实测「点保存并释放 → 下一张出现」≈ **0.78s**，拆开：
| 环节 | 耗时 | 占比 |
|---|---|---|
| **5 张增强图渲染** | **0.64s** | **82%** |
| 下一张显示图冷渲染 | 0.14s | 18% |
...
再拆显示图那 0.14s：读原始道集 0.005s、预处理 0.001s、**matplotlib 渲染落盘 0.106s**
—— 瓶颈全是画图，不是 I/O。所以「把数据缓到内存」这类方案没用；用户原本设想的
「预热后几张显示图」最多只值 18%，还得先解决 `claim_pool` **随机抽**导致"不知道下一张是谁"
（出路是让抽取从预渲染好的 k 个候选里挑，分布不变，但要改抽取路径、照顾跳过队列与租约）。

**真正的答案**：那 5 张增强图是给**训练导出/云回传**用的，跟「下一张标什么」毫无关系，
却卡在保存请求里同步渲染。

**改法**（用户选 A：先把这 82% 挪走，不动抽取语义）
- `JobManager.save` 拆两段：**记录（基础 + 5 条增强）与 npy 同步落盘**（纯 JSON，~5ms），
  **PNG 交给后台线程**。记录必须同步 —— 断点续标/进度/重开读的都是 labels.jsonl。
- 新增 `AugRenderQueue`（守护线程 + 按 (job_id, gid) 去重、后来者覆盖），照搬
  `Uploader` 的后台线程模式。单线程是有意的：并发保存本来就是各请求线程在并发调
  matplotlib，串行反而更保守；吞吐约 5×0.1s ≈ 0.5s/张。
- **云回传跟着挪到「图画完」的回调**（`JM.set_aug_hook`，web_app 用 `make_jm` 装配；
  回调里惰性取模块级 `UP`，测试替换 `web_app.UP` 后仍生效）。jobmanager 不依赖 cloudsync。
- **过期任务防呆**：画每张前先查 `store.get(aug_id)` 还在不在 —— 重开修正会先
  `remove_augmented`，在途的旧任务必须认出自己作废，否则会把刚删掉的旧图**又画回来**，
  images/ 里就多出没有记录对应的孤立图。
- **崩溃兜底**：进程在画图途中被杀 → 记录在、图缺。启动后起一个**不阻塞启动**的后台扫描
  `sweep_missing_aug()` 补画；离线仍有 `backfill_augment.py`。
- 测试契约：新增模块级 `AUG_ASYNC_DEFAULT`，测试模块置 False 一行切同步（否则
  「存完图就在」「images/ 应为空」这类断言会偶发挂）。smoke 测试改用 `web_app.make_jm()`
  建实例，保证钩子在场、上传那一路真的被覆盖到。

**验证**
- 端到端实测：**0.814s → 0.168s（省 79%，快 5 倍）**。用户环境是「几秒级」，按同比例
  应降到几百毫秒。
- `pytest` → **163 passed**（新增 `TestAugRenderQueue` 8 项：保存不等渲染、排空后图与记录
  一一对应、钩子在图存在后才调且带上 labels.jsonl、重存不留孤立图、补画缺失图、
  无事不排队、单张失败不拖垮线程）；`smoke_web.py` → **54 passed**；`build_app()` OK；
  `node --check` OK；真起服务启动无异常。
- **变异测试**：把 6 处逐个撤掉确认变红 —— 其中**代号（generation）检查是空转的**
  （记录存在性检查已经覆盖了它），据此**删掉了那段代码**，不留测不到的复杂度。
  还逮到两个自己写的 bug：同步模式下钩子从没被调（上传整批丢失）、重构时弄丢了
  `labels.jsonl` 的上传项 —— 都是既有测试抓到的。

**遗留问题**：保存刚返回的几百毫秒内 `images/` 尚缺那几张图（属预期，提示语已写明）。
极端并发（两个标签页同存同一道集）下仍有微秒级窗口可能补出一张孤立图，
靠「记录存在性」判据已把窗口压到极小。

---

## v3.13.2（2026-09-12）— 撤销 v3.13 的「修正保存原地停住」：保存并释放一律前进

**用户实测反馈**：点「返回上一张」修正完再点「保存并释放」，**没有自动跳到下一张**。

**这是我自作主张加的行为**（v3.13 设计阶段提过、用户当时也点头了）：修正保存原地停住，
理由是这样连续往回翻才连贯 —— 否则每改完一张就被丢到随机新道集，再点「返回上一张」
又回到刚改完的那张。**但实际用起来是错的**：按钮就叫**「保存并释放」**，只覆盖不前进，
在用户眼里就是"卡住了"。按钮名字许下的承诺比这条推理更有分量。

**修法**：撤掉 `was_labeled` 的提前返回，修正保存**照常自动领下一张**；「这是覆盖不是新增」
改由**提示语**讲清楚（`✔ 已覆盖「G」的原结果 × N 张增强图 | … | 已自动领取下一张「R」`
vs 首次的 `✔ 已保存「G」…`）。保存过的那张照旧进浏览历史，「返回上一张」仍能回到它。

**验证**：按用户的操作序列跑了一遍端到端 —— 领 G2 → 存（领 G3）→ 返回上一张（归还 G3、
回到 G2）→ 改一项 → 保存并释放 → **落到新的一张 ✔**，且 G2 的 `gather_type` 确实被改成
「残差」、`annotated_by` 仍是 ann1、**标注总数仍是 1**（覆盖而非新增）。
`pytest` → 155 passed；`smoke_web.py` → **54 passed**（`SmokeBackPrevious` 里那条改成
断言"覆盖 + 仍前进"，`SmokeWebMultiUser` 两条恢复只认「已覆盖」）。

**教训**：把"用户体验上更连贯"的推理强加给一个名字已经写明语义的按钮，是过度设计。
按钮叫什么，就该干什么。

---

## v3.13.1（2026-09-12）— 修 v3.13 的两处实测问题：右键清不掉已落定的框、返回上一张不生效

**用户实测反馈（上版上线后立刻发现）**
1. 右键只清得掉「还没闭合的半成品」，**已落定的框清不掉**；
2. 点「返回上一张」没反应，**还停在当前张**。

#### ② 「返回上一张」不生效 —— 是我加的守卫把按钮变成了摆设

v3.13 里给 `back_previous` 加了「手上那张未保存就先拦下」的守卫（照抄「领取下一张」的
思路）。但保存之后程序**总会自动领下一张**，于是用户一点「返回上一张」，站着的那张必然
是「未保存」的 → 每次都被拦下 → 表现成按钮没反应。

**修法**：区分「未作答」和「已作答」——
- 刚领来一个字都没填（最常见，就是保存后自动领的那张）→ **直接 `JM.release` 归还回池
  再往回走**，提示里写明归还了哪张；
- 真答过了（`_has_unsaved_input`：`st['partial']` 有值 或 `st['boxes']` 有非空框）→ 才拦下
  问一句，避免把真实工作静默丢掉。

服务端状态忠实反映用户输入（改选项走 `on_radio_change` 写 `partial`，画框走 `/api/anno_box`
写 `boxes`），所以「有没有作答」判得准。

#### ① 右键清不掉已落定的框 —— 不该去猜 Gradio 按钮的 DOM

v3.13 的右键清框是**程序化点**该特征的 `#cb-<key>`（「✕ 清除此框」）。这条路在真页面上
走不通：`elem_id` 落在 `<button>` 还是外层 `div`、各版本/Gradio 6 的组件树都不一样，
`getElementById` + `querySelector('button')` 猜不中就静默什么都不做。

**修法**：不再依赖 DOM —— 新增 `clear_drag_box()` + `POST /api/anno_box_clear`，与
`/api/anno_box`（Ctrl 两点提交）同一套路。前端 `onCtxMenu` 改为 `clearKey(key)`（本地立刻
消失、点了就有反馈）+ `postClear(key)`（服务端同步清掉 → 清完回读目标）。
**并顺带修了一个我自己埋的时序坑**：`postClear` 先 `clearTimeout(refreshTimer)` 取消还没到点的
去抖回读 —— 否则它可能赶在服务端清掉**之前**发出，把刚清掉的框又拉回来（`clearKey` 的注释里
记过同一类坑）。

**验证**
- `pytest` → **155 passed**；`pytest tests/smoke_web.py` → **54 passed**（较 v3.13 +4：
  已作答才拦下、只画了框也算已作答、`clear_drag_box` 清已落定的框 + 目标顺延、非法 key/未领取）。
- `tests/test_anno_js.py` 的右键一节改写为「本地立刻清 + 直连 clear 接口 + 清完回读 +
  清框前必须取消去抖回读」5 个分支。
- **变异测试**：5 处（退回程序化点按钮 / 不取消去抖 / 清完不回读 / 守卫恒拦 / 作答判定恒假）
  逐个撤掉，对应断言**全部变红**。
- **HTTP 实测**：起真服务打新接口 —— 未知用户 401、合法用户未领道集返回
  `{"ok":false,"msg":"没有正在标注的道集"}`（不是 404，证明路由真的挂上了）。
- 顺带补：Node 桩原先没有 `clearTimeout`（`postClear` 会直接抛），是这条测试暴露的。

**遗留问题**：无新增。仍未经真浏览器实点（环境无 playwright/selenium，Gradio 6 又是纯客户端
渲染，服务端吐出的 HTML 里不含组件 DOM）—— 交互状态机靠 Node 桩 + 变异测试覆盖。

---

## v3.13（2026-09-12）— Ctrl 松手成框、右键清框、返回上一张、修复「不存在仍能拉框」

**用户反馈（4 项）**：①面波/近炮点拉框希望按住 Ctrl 点一个点、松开 Ctrl 就自动定第二点；
②拉框状态下点右键清除此框；③加「返回上一张」按钮，回去修正后重新保存覆盖原结果；
④**bug**：两项都选「不存在」却仍能拉框；先都选「不存在」再把某项改回「存在」，反而提示
不存在无法拉框。点名要查「继承最近已标注」相关部分。

#### ④ 根因（不是 box_target 的错，是前端读得太早）

`web_core.box_target` 本身正确，`finalize_regions` 保存时也早就会把「不存在」特征的框置空
—— 纯 UI 状态陈旧。链条是：

- Ctrl 目标（`targetKey`/`targetNote`）**只**由 `/api/boxes` 回读，而回读的唯一自动触发点是
  document 上的 `change` 监听。该监听比 Gradio 的 `on_radio_change`（写 `st['partial']` 的那个）
  **先跑**，读到的必然是改之前的选项；
- 等服务端跑完，`on_radio_change` 的 outputs 里**没有图片**，`place()` 不触发 → **再没人回读**。
- 于是 target/note 永久慢一拍：设完「面波=不存在」画布仍写「框选：面波」；再设「近炮点=不存在」，
  画布才读到上一拍的 partial，把目标落到近炮点上 → **还能拉出一个框**（症状 A）；此时把面波改回
  「存在」，画布读到的是「两项都不存在」那一拍 → **提示不存在无法画框**（症状 B）。
- 「⧉ 继承最近已标注」更彻底：它用 `gr.Radio(value=…)` **程序化**改选项，压根不产生 DOM change
  事件，且 outputs 里没有 `box_statuses` —— 继承完画布目标从没刷新过。这就是用户让查的那部分。
- 旁证：`clearKey` 的注释（"不能立刻 fetchBoxes()，服务端要等本次 Gradio 事件跑完"）说明
  同一个时序坑此前已经踩过一次。

**修法**
- 回读触发点从「客户端 change 事件」换成「**服务端 outputs 落到 DOM 之后**」：复用
  `#anno_featcol` 上本就为补画高亮而挂的 MutationObserver，在里面加**去抖**的
  `scheduleTargetRefresh()`（60ms 合并同一批渲染）。选项类 outputs 正是写进这一列的。
- 删掉 change 监听里那句过早的 `fetchBoxes()`（它只可能读到旧状态）。
- `fetchBoxes()` 在 `grab`（拖动/缩放）期间**不替换 boxes**，只更新 target/note ——
  否则去抖回读会把手上正在拖的框打回服务端旧值。
- `inherit_previous` 的 outputs 补上 `*box_statuses`：既修「继承完框状态标记也是旧的」，
  又保证这一路同样产生刷新信号。

#### ① 松开 Ctrl 成框
`onDown` 里「第二角成框」抽成 `commitBox`，与新增的 document `keyup` 共用一份实现。
`keyup` 命中 `Control`/`Meta`/`OS` 且 `pending && hoverNat` 时，以**松开瞬间的鼠标位置**作第二角。
`onDown` 里补记 `hoverNat = nat`（原来只在 onMove 记），否则「点了不移动就松手」时位置是陈的。
框太小（<MIN）→ **不成框且 pending 原样保留**，不会因一次误松手丢掉第一角；指针已离开图片
（hoverNat 被 onLeave 清空）同样保留。原来的「Ctrl 再点第二角」完全不变。

#### ② 右键清框
放在 `contextmenu`（不是 pointerdown）：能顺手 `preventDefault` 掉浏览器菜单，也不打扰
pointer 手势状态机。规则：有待定框 → 丢弃它；无待定框且鼠标压在某个框上 → 程序化点该特征的
`#cb-<key>` 按钮（复用既有服务端清除路径及那里的时序规避）；空白处 → 什么都不做。
顺带在 `onDown` 加 `if (e.button) return;` —— 原来右键落在已有框上会被 `hitTest` 命中、
白白启动一次"拖动"并捕获指针。

#### ③ 返回上一张
- 新增纯函数 `web_core.prev_gid(hist, cur) -> (目标, 回退后的 hist)`。
- 历史用**浏览栈** `st['hist']`，不用「我标注的」列表顺序：那张表按**首次标注时间**排序
  （`LabelStore.upsert` 对已存在的 gid 不挪位），修正保存不会让记录回到末尾，靠它会翻错张。
  压栈点集中在 `_visit()`（换道集时把原来那张压入）；保存流程会先 pop 掉 gid，故
  `save_anno` 显式 `_push_hist(st, gid)` 再领下一张，否则刚标完那张会漏出历史。
- ~~**修正保存原地停住**：保存前该道集已有记录 → 不自动领下一张。~~
  **v3.13.2 已撤销**（实测被当成"卡住了"）：保存并释放一律前进，只在提示语里区分覆盖/新增。
- 手上那张未保存 → 先拦下并给指引（与「领取下一张」同一套），不静默丢工作。
  **v3.13.1 修正**：只有**已作答**的才拦；一个字没填的直接归还回池再往回走
  （保存后自动领到的那张必然未作答，一律拦下会让按钮在最常用场景里失效）。
- 换作业选择（`select_jobs`）清空 hist：显式换了上下文。到最早一张 → 明确提示。

**验证**
- `pytest` → **155 passed**（新增 `TestPrevGid` 7 项）。
- `pytest tests/smoke_web.py` → **50 passed**（新增 `SmokeBackPrevious` 5 项 +
  `SmokeInheritRefreshesBoxes` 2 项；`SmokeAnnoOutputArity` 补 back_previous 并改
  inherit 的应有长度）。
- `tests/test_anno_js.py` 扩了 Node 桩（MutationObserver / 可控 setTimeout / 可同步 resolve
  的 fetch / 可跑通的 setup+place），新增 3 节断言：松手成框的 5 种分支、右键 3 态、
  「featcol 变更才回读 + 去抖 + 拖动中不回填」。
- **变异测试**：把 6 处修复逐个撤掉，确认对应断言**全部变红**（含「把过期的 fetchBoxes()
  加回 change 监听」由 `test_target_refresh_waits_for_server_outputs` 的源码锁拦住）——
  不这么做的话，这几条很容易写成"怎么改都过"的空转测试。

**遗留问题**：无新增。前端回读依赖「sentence/box_statuses 确实被重写」产生 DOM 变更；
若两个不同选项的 phrase 完全相同（label_config 支持 phrase，当前配置未用），
继承那条路可能不产生变更 —— 当前配置下不会发生。

---

## v3.12（2026-09-12）— 排查「已完成任务被重发」（实为误导性提示）+ 空态清图

**用户反馈**：标注者反映分到的图上左上角有「框选已完成（Ctrl 可重画）」，怀疑已完成的
任务又被分配了。

**实测结论：没有重发。** 写了排查脚本跑三轮，已标注道集一次都没再被发出：
- 单作业标一半后反复领取/归还 **30 次** → 命中已标注的：**0**
- 重启（`load_all` 重新注册作业）后再领 20 次 → **0**
- 全部标完后领取 → `gid=None`，reason「选中的作业已全部标注（n/n）」

**真正的原因（两件事凑一起造成的错觉）**
1. `claim_next` / `_claim_from_pool` 在池子标完时走 `_anno_idle()`，而它对图片返回
   **`gr.skip()` —— 旧图留在屏幕上不刷新**；
2. 此时没有当前道集，`/api/boxes` 的 `target` 为 `None`，而前端把「target 为空」一律
   画成 **「框选已完成（Ctrl 可重画）」**。
→ 标注者看到的就是「最后那张已标注的图 + 上面盖着一句『框选已完成』」，像"又发了一张"。

同一条提示还有第二种误导：当前这张两项都选「不存在」时 `target` 也是 None，
会显示"框选已完成"，其实只是"无需画框"。

**修法**
- **空态清图**：`_anno_idle()` 的图片输出由 `gr.skip()` 改成 `gr.update(value=None)` ——
  没有当前道集就把图清掉，旧图不可能再冒充"新分配的"。radio 与滤波参数仍保持不动。
- **文案交给服务端**：`/api/boxes` 增加 `note` 字段（`web_core.box_note()` 纯函数）——
  target 为空时给出**准确**说明（两项都「不存在」→「无需画框」；没有带框特征 →
  「本题不需要画框」；**没有当前道集 → 空串**）。前端 `drawBoxHint` 改为：有目标显示
  "Ctrl+左键点两角框选：X"，有 pending 显示"再点第二角"，否则**只在 note 非空时才画**，
  `targetNote` 为空就什么都不画。「框选已完成」这句话从代码里删掉了。

**验证**：`pytest` → **147 passed**；`smoke_web.py` → **43 passed**；`build_app()` OK；
`node --check` 通过。新增 `SmokeNoReassignAndHint` 6 项把这次的结论钉住：
①标完后再领 20 次都拿不到道集、②重启后同样拿不到、③空态**图片必须被清空**（不能再是 skip）、
④无当前道集时 note 为空串、⑤两项都「不存在」时 note 说明"无需画框"、⑥**前端代码里
不允许再出现「框选已完成」**且必须用 targetNote。`TestBoxNote` 4 项覆盖文案分支。

**教训**：提示语不能替用户下结论 —— "框选已完成"在"无需画框"和"没有道集"两种情况下都会
被显示出来，而这两种情况都**不是**"完成"。文案应由掌握状态的**服务端**给出，前端不要自己猜。

---

## v3.11（2026-09-11）— 本用户标注总量 + 管理员看各用户标注总量

**需求（用户）**：标注区增加「本用户的标注总数量，不区分作业」；每个用户的标注总量也统一
显示在管理员的管理区。

**计数口径（`JobManager.user_counts()` / `user_count(user)`）**
- 只数**基础记录**（`image_path is None`）：一次保存写 1 条基础 + N_AUG 条增强，而增强记录
  同样带 `annotated_by`，不过滤会把张数算成 ×(N_AUG+1)（= 6 倍）。
- **跨全部作业**累加；**软删除的作业不计**（与 `list_jobs()` 口径一致 —— 相邻两处显示
  不该互相打架）。
- 他人覆盖重标同一张时 `save` 保留原标注者 → 归属稳定，总量不会平移（有测试钉住）。

**显示**
- 标注区：`my_total_md` 常驻 markdown（`_my_total_text(user)`），作为 **anno_outputs 末位**
  输出，保存/领取等每个 anno 事件都会刷新；`init_page` 也把它带上，页面一加载就是准的
  （组件初值只是"服务启动那一刻"的快照，和进度表一个道理）。
  空态走 `gr.skip()` 保留原值 —— 计数没变，不必重算、更不能洗成 0。
- 管理区：④ 里新增 `staff_progress = gr.HTML(staff_counts_html(...))`，列出 **users.yaml 里
  全部账号**（含 0 张的，`_staff_rows()` 用 `ACC.names()` + `ACC.role()`）并给出合计；
  随 `refresh_jobs_progress`（5 秒）与删除/恢复事件一起刷新。
- `web_core.staff_counts_html(rows)` 做成纯函数便于单测（转义用户名、坏值当 0、空表提示）。
- 加了 `.staff-counts` 样式，与 `jobs-progress` 观感一致（右对齐数字、合计行加粗）。

**踩到的两个点**
1. `_anno_idle` 里该字段用 `gr.skip()` 而非重算 —— 测试一开始按"空态也要显示新值"写，
   实际上空态计数本来就没变，skip 才是对的（已把测试改成断言 skip）。
2. `_staff_rows()` 读的是**真实 users.yaml**，写测试时不能假定里面有 ann1：
   `SmokeUserTotals` 现在会在 setUp 里把 `web_app.ACC` 换成临时账号表（与前面对 JM/UP 的做法一致）。

**验证**：`pytest` → **143 passed**（新增 `TestClaimPool` 3 项：跨作业累加且只数基础记录
（断言 `len(augs)==N_AUG` 同时计数为 1）、覆盖后归属不变、删除作业不计入；
`TestStaffCountsHtml` 5 项）；`smoke_web.py` → **37 passed**（`SmokeUserTotals` 5 项：
标注区显示并在保存后 +1、连续保存累加、空态 skip不清零、staff 表含全部账号且 0 张也列出、
用户表确实在 ④ 管理区之后）；`build_app()` OK。
实测真实数据渲染：boss 9 / hp 38 / lq 12 / 其余 0 / 合计 59。

---

## v3.10.1（2026-09-11）— 修复网页上下滚动卡顿

**用户反馈**：网页上下拖动时卡顿。

**三个成因（都在前端热路径上，一眼能看出为什么"只在滚动时"卡）**
1. **`window.addEventListener('scroll', … place(), true)` 未节流**：`place()` 里每次都会
   `canvas.width = …` —— 给 canvas 赋尺寸会**重建画布缓冲并清空内容**，再叠加 `redraw()`
   全量重绘（含 `measureText`）。于是每个 scroll 事件都做一次，必卡。
2. **`place()` 里顺手 `paintCursor()`，而它含 `scrollIntoView({block:'nearest'})`**：
   当前题不在视野内时会把页面拽回去 —— 等于跟用户的滚动**抢方向盘**，被拽回的那一刻
   体感就是"卡"。而且它挂在 scroll / 900ms 定时 / 每次 DOM 变化上，反复发生。
3. **MutationObserver `attributeFilter:['src','class']` 且监听整个子树、未节流**：
   Gradio 频繁切换 class（pending/generating/selected）就会触发一次完整 `place()`。

**修法**
- `place(force)` 增加**几何签名比对**（`rectSig(ir, cr)`）：签名没变直接 return。
  canvas 与 img 同在一个 `position:relative` 容器里，页面滚动时两者一起移动、相对位置不变
  → 平静滚动现在只花两次 `getBoundingClientRect`，**零重排、零重绘**。
- canvas 尺寸赋值前判等（`if (canvas.width !== w2)`），避免无谓清空画布。
- 新增 `raf()` 把一帧内的多次请求合并；scroll / resize / MutationObserver / load / 定时 全部走它。
- `paintCursor(scrollIntoView)` 加参数：**只有用户主动移动光标**（`handleDigit` / `moveCursor`）
  才滚进视野；重绘与 DOM 变化补画时不滚 —— 不再跟用户滚动抢方向盘。
- `paintCursor` 从 `place()` 里摘出来，改由**题目区的 childList 观察者**驱动（Gradio 重渲染
  会丢 `.anno-current` 高亮，需要补画），这样滚动/定时路径不再跑那 7 次 DOM 查询与 class 切换。
- 观察者 `attributeFilter` 去掉 `class`，只留 `src`；另加一个只盯 `#anno_featcol` childList 的观察者。

**验证**：`pytest` → **135 passed**；`smoke_web.py` → **32 passed**；`build_app()` OK；
`node --check` 通过。新增 `test_scroll_hot_path_stays_cheap` 作为回归锁（静态断言）：
place() 必须有签名守卫、滚动监听必须经 raf()、**不允许存在无参 `paintCursor()`**、
观察者不得再听 `class`、canvas 尺寸必须判等。
**遗留**：真实帧率仍需浏览器实测确认（我无法在这里量 FPS）；若仍卡，下一个可疑点是
`gr.Timer(5s)` 每次替换进度表 HTML 引起的整页重排。

---

## v3.10（2026-09-11）— 多作业作业池：多选作业，从池里随机抽道集

**需求（用户）**：标注者可同时添加多个作业，每次道集从这些作业组成的池子里随机抽取。
（拍板：抽样按**道集数量比例**；**管理员与标注者都是多选**；顶部**同时显示当前作业与池汇总**。）

**`JobManager` 新增**
- `_free_gids(job_id, user, skip_aware)`：单作业的可领道集枚举（未标注/未被占/可避开本人近期跳过），
  顺带回收过期租约 —— 从 `claim` 里抽出来的公共部分。
- `claim_pool(job_ids, user)`：**摊平所有选中作业的可领道集 → `random.choice` 等概率抽**
  （= 按道集数比例，作业越大抽中越多）。三层语义：
  1. 已在任一选中作业持有有效租约 → **resume 那一张**（切选择/刷新不丢未保存工作）；
  2. 先只从「本人近期没跳过的」候选抽，抽不到才回到跳过项（消费其避让）——
     保留单作业版的**防振荡**语义，否则一跳过就可能立刻又抽到同一张；
  3. 池空 → 明确原因（全标完 / 都被占 / 未开放），不静默失败。
- `skip_release(job_id, user)`：只做「放回 + 记跳过」，抽下一张交给调用方 ——
  **多作业下必须再从整个池抽**，沿用单作业的 `skip()` 会抽不到别的作业。
  （旧 `skip()` 保留原样包一层 skip_release+claim，兼容既有测试。）
- `pool_progress(job_ids)` → (已标, 总数, 作业数)；`_raw_progress` 从 `progress` 抽出复用。

**`web_app` 改造**
- `job_dd` → `multiselect=True`；所有 job_dd 的 `value` 从 `None/字符串` 改成 **list**
  （多选组件必须给列表，共 7 处，其中 `create_job_ui` 建完作业勾选 `[新作业]`）。
- 工作态新增 `st["job_ids"]`（选择）；`st["job_id"]` 改为**当前道集所属作业**，随每张道集走
  —— 保存/进度/滤波参数都按它，所以两个作业能交替标注。
- `open_job_for` → 重命名 `select_jobs`（参数变列表）；新增 `_claim_from_pool()` 统一
  「从池领一张 + 返回一句状态说明」，`select_jobs / claim_next / save_anno / skip_current` 共用。
  返回值是 `(anno_outputs 形状, 一句说明)` —— 说明用于拼消息，避免把整行道集信息塞进保存提示。
- `reopen_mine(job_ids, gid)`：新增 `_job_of_gid()` 在选中作业里定位该 gid 属于谁
  （**同文件建的两个作业会产生相同 gid，此时取第一个**，已在代码注释说明）。
- `_mine_choices` 改为选中作业的**并集**；`refresh_page` 会把已删除/关闭的选中项收敛掉，
  且**当前道集所属作业被取消勾选时清空当前道集**。
- 顶部进度 `_progress_text()`：`本作业 a/b`，多选时再追加 `｜ 作业池 c/d（共 n 个作业）`。

**验证**：`pytest` → **133 passed**（`TestClaimPool` 9 项：真实抽到、空选择提示、resume、
**抽样比例**（4:8 的池子里重抽 600 次，B 命中 > A×1.4 —— 放宽阈值以稳定，但足以挡住
「每作业等概率」）、他人租约占满、全标完提示、跳过避让+回退、池汇总、关闭/未知作业）；
`smoke_web.py` → **31 passed**（`SmokeJobPool` 7 项：两作业选到其一且顶部含池汇总、
单选不显示池汇总、**连续保存能跨到两个作业**、跳过换张、同池重选保持当前张、
取消勾选当前作业即清空、`我标注的` 并集与 gid 归属定位）；`build_app()` OK，
实测 config 里 `multiselect: true`、所有 job_dd 的 value 均为列表。

**遗留**：池子按道集数比例抽，小作业会被大作业「稀释」（用户明确选了这一口径；
若要每个作业机会均等，只需把 `random.choice(pool)` 换成先抽作业再抽道集）。

---

## v3.9（2026-09-11）— 标注量/作业进度对全体可见

**需求（用户）**：管理员和标注者都能看到标注量。

**改动**
- 把「作业进度一览」（`jobs_progress_html`）从**仅管理员可见**的「③ 管理」里搬出来，
  独立成全体可见的「**③ 标注量 / 作业进度**」（`open=True`），管理动作（全部重传 /
  删除 / 恢复 / 账号管理）留在新的「**④ 管理（仅管理员）**」。
- **Timer 必须跟着搬到可见区**：`gr.Timer` 原先声明在仅管理员可见的 accordion 里，
  标注者那边该容器不渲染、定时器可能根本不触发（管理员侧一直没问题，所以从没暴露）。
  现在 Timer 声明在 ③ 区，`tick` 的输出仍是 `[mgr_progress, btn_delete, del_info]` ——
  后两个对标注者是不可见组件，照旧更新无副作用（这个"更新隐藏组件"的用法在
  `init_page` 里本来就有先例）。
- **顺带修掉一个陈旧数据问题**：`mgr_progress` 的初值是 `build_app()`（＝**服务启动那一刻**）
  的快照，而 `init_page` 原本不输出它 → 新开的页面会看到过期计数，等第一次 5 秒 tick 才修正。
  现在 `init_page` 多返回一个 `jobs_progress_html(JM.list_jobs())`（输出列表同步加到 7 项），
  页面一加载就是最新数据。
- 表内只有作业级汇总（标题/ID/状态/已完成/总数/百分比），**不含标注者姓名**
  （`_info()` 虽有 `created_by`，但 `jobs_progress_html` 不渲染它），
  所以对标注者不构成"谁标了多少"的暴露。
- README 同步：角色说明里写明 ③ 区对两种角色可见、「管理③区」的旧说法改成「管理④区」。

**验证**：`pytest` → **124 passed**；`smoke_web.py` → **24 passed**（新增 3 项：
③ 标注量区 visible=true、① 建作业 / ④ 管理 仍 visible=false、
`init_page` 输出 7 项且末项为最新进度表 HTML —— arity 对不上就会在页面加载时报错）；
`build_app()` OK。另实测各 accordion 的 label/visible 与预期一致。

**遗留**：标注者现在能看到**所有**作业的进度（含他们打不开的已关闭作业）——
五人小团队内可接受；若要严格按可见范围过滤，可在 `_job_choices` 的基础上过滤表数据。
表里也没有"每个人各标了多少"的维度，需要的话可加。
