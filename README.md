...
- 条件标签体系：炮集标噪声存在情况，残差标噪声类型与去噪质量
- **键盘一路答完选择题**：当前题高亮显示，按数字键 `1`～`9` 选中对应选项后**自动跳到下一题**，
  `↑`/`↓` 可前后移动当前题；标注时基本不用鼠标点选项
- 异常振幅 / 面波 / 近炮点强能量噪声支持不规则包络：按住 `Ctrl` 依次点击多个点，松开完成
...
- 标注区左侧提供 90～99.9 的水平显示 clip 滑块；调整只影响当前预览，保存的图片和标注结果不变。
  最近一次调整会在同一作业内后续道集中沿用，并对不同标注者共享。
...
集合类型为**炮集**时，后续问题为异常振幅、混叠噪声、面波、近炮点强能量噪声，并提供对应包络项。
集合类型为**残差**时，第 2 题为噪声类型（可多选），第 3 题为去噪质量；噪声残留和信号泄露可同时选，
去噪完成为独占选项。

**① 键盘答选择题（1～4）** —— 当前题有**蓝色高亮框**（整块标题+选项发光），一眼看得到在答哪道。
...
- `↑` 到上一题、`↓` 到下一题（可回去改答案；改完按数字键会覆盖原选择）。
  ...
  默认的「同组内换选项」行为抢走按键）。
- 到拉框项（5/6/7）时数字键不再动作。在文本框里打字不影响（数字键照常输入）。
- **选了「不存在」的那一步自动跳过**：异常振幅、面波或近炮点强能量选「不存在」时，
  对应拉框项会被直接跳过（该步置灰、状态行写「无需画框（本步自动跳过）」）。改回「存在」即恢复。
- 全部答完（含跳过的框选步骤）后按 **`Enter`** 即「保存并释放」并自动领取下一张，
  全程不用碰鼠标。焦点在输入框里时 `Enter` 不触发保存。

**② 鼠标包络** —— 对应的选择题选了「不存在」时，这一步会被自动跳过（见上）。
- 在图上按住 **`Ctrl`**（Mac 为 `Cmd`）依次点击至少 3 个边界点，松开 `Ctrl` 后自动闭合并保存不规则包络。
...
```json
{
  ...
  "gather_value": 1373,
  "n_traces": 528,
  "labels": {"gather_type": "炮集", "abnormal_amplitude": "不存在", "surface_wave": "已压制但有残留", "near_shot_noise": "不存在"},
  "sentence": "这是一条炮集，异常振幅不存在，面波已压制但有残留，近炮点强能量噪声不存在。",
  "regions": {"abnormal_amplitude": null,
              "surface_wave": {"xyxy": [100, 200, 300, 600], "traces": [103, 311], "samples": [781, 2348]},
...
- `when` 控制集合类型分支，`input: checkbox` 定义多选题，选项的 `exclusive: true` 定义独占项
- `bbox: true` 的特征会生成独立的不规则包络项
